...
#
# codex was briefly off (2026-08-14) after its first warm implementation lost
# conversation continuity in production: it republished codex APP-SERVER
# JSON-RPC frames, which agents-platform's consumer does not parse, so no
# thread id was ever captured and every turn opened a new conversation AND a
# new container. It is back on with aw-warm-relay-codex.py rewritten to run
# one `codex exec resume --json` per turn instead, whose stdout is the exact
# stream the cold path produces — the consumer cannot tell the two apart, so
# session continuity works for the same reason it always did.
